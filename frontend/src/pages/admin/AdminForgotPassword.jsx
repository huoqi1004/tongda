...
import { User, Lock, ShieldCheck, ArrowLeft, Eye, EyeOff, CheckCircle2, Clock } from 'lucide-react'
import api from '../../api'

const STEPS = [
  { key: 'account', title: '验证账号', desc: '请输入管理员用户名' },
  { key: 'verify', title: '输入验证码', desc: '请输入发送到管理员账号的 6 位验证码' },
  { key: 'reset', title: '重置密码', desc: '请设置一个新的管理员登录密码' },
]
...
export default function AdminForgotPassword() {
  ...
  const sendCode = async () => {
    if (!account.trim()) { setError('请输入管理员用户名'); return }
    setLoading(true); setError('')
    try {
      const res = await api.post('/admin/forgot/send-code', { account: account.trim() })
      const data = res.data?.data || {}
      setMaskedAccount(data.maskedAccount || account)
      if (data.devCode) setDevCode(data.devCode)
      setCooldown(Math.ceil((data.cooldown || 60)))
      setStep(1)
      setSuccess(`验证码已发送至管理员账号 ${data.maskedAccount || account}`)
  ...
  const verifyCode = async () => {
    ...
    setLoading(true); setError('')
    try {
      const res = await api.post('/admin/forgot/verify-code', { account: account.trim(), code: full })
  ...
  const submitReset = async () => {
    ...
    setLoading(true); setError('')
    try {
      await api.post('/admin/forgot/reset', { resetToken, newPassword: newPwd })
    ...
    <div className="min-h-screen bg-gray-100 flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8">
            ...
            {STEPS.map((s, i) => (
              <div key={s.key} className="flex-1 flex items-center">
                <div className="flex flex-col items-center flex-1">
                  ...
                  <div className={`mt-1.5 text-[10px] ${i <= step ? 'text-td-dark font-medium' : 'text-td-silver'}`}>
                    {s.title}
                  </div>
                </div>
                {i < STEPS.length - 1 && (
          ...
          <p className="text-xs text-td-gray mb-4 text-center">{STEPS[step].desc}</p>

          {error && (
            <div className="bg-red-50 border border-red-200 text-red-600 text-xs rounded-lg px-3 py-2.5 mb-4">{error}</div>
          )}
          {success && (
            <div className="bg-green-50 border border-green-200 text-green-700 text-xs rounded-lg px-3 py-2.5 mb-4">{success}</div>
          )}
          ...
          {step === 1 && (
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-1.5 justify-between" onPaste={handleCodePaste}>
                  {code.map((c, i) => (
                    <input
                      key={i}
                      ...
                      className="w-10 h-10 text-center text-lg font-semibold border border-gray-300 rounded-lg focus:ring-2 focus:ring-td-red focus:border-td-red outline-none"
                    />
                  ))}
                </div>
