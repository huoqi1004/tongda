...
export default function ForgotPassword() {
  ...
  const verifyAnswer = async () => {
    ...
    setError('')
    try {
      const res = await api.post('/security-questions/verify', {
        phoneOrEmail: account.trim(),
  ...
  const submitReset = async () => {
    ...
    if (!passwordRegex.test(newPwd)) {
      setError('密码不符合要求：至少8位，包含大小写字母、数字和特殊字符')
      return
    }
    ...
    setError('')
    try {
      const res = await api.post('/security-questions/reset', {
        phoneOrEmail: account.trim(),
        questionId: parseInt(selectedQuestion),
        answer: answer.trim(),
        newPassword: newPwd
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8">
          ...
          {step === 0 && (
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-td-dark mb-1.5">注册手机号/邮箱</label>
